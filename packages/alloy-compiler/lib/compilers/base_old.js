...
module.exports = class BaseCompiler {
	...
	findAllModels() {
		...
		widgetDirs.forEach(collection => {
			const widget = this.findWidget(collection.dir);
	...
	createTemplateObject(componentMeta) {
		...
		return {
			...
			Widget: !manifest ? '' : 'const ' + CONST.WIDGET_OBJECT
				...
				+ manifest.id + '\';',
			WPATH: !manifest ? '' : _.template(fs.readFileSync(path.join(this.config.dir.template, 'wpath.js'), 'utf8'))({ WIDGETID: manifest.id }),
			ES6Mod: ''
		};
	}
	...
	compileComponent(options) {
		...
		let codeTemplate = _.template(fs.readFileSync(path.join(this.config.dir.template, 'component.js'), 'utf8'))(template);
	...
	compileModel(options) {
		...
		const baseDir = isWidget ? widget.dir : this.appDir;
		...
		const code = _.template(fs.readFileSync(modelTemplateFile, 'utf8'))({
			basename,
			modelJs: content,
			migrations
		});
	...
	findModelMigrations(name, migrationDir) {
		let files = [];
		try {
			files = this.fs.readDirSync(migrationDir);
		} catch (e) {}
	...
	compileStyle(options) {
		...
		if (manifest) {
			styleCode += _.template(
				fs.readFileSync(path.join(this.config.dir.template, 'wpath.js'), 'utf8')
