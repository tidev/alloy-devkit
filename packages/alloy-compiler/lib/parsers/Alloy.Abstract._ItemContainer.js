...
function parse(node, state) {
	...
	_.each(children, function (child) {
		...
		if (_.find(def.children, function (c) {
			return c.name === theNode;
		})) {
			var childState = {
				parent: {},
			...
			var prop = _.find(def.children, function (c) {
				return c.name === theNode;
			}).property;
			extras.push([ prop, childState.itemsArray ]);

			// Only add the extraOptions if they are defined on the child nodes
			_.each(U.XML.getElementsFromNodes(child.childNodes), function (node) {
				_.each(state.extraOptions, function (varName, name) {
					var attr = _.find(node.attributes, [ 'nodeName', name ]);
					if (attr !== undefined) {
